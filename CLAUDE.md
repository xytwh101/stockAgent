...
│   ├── scorer.py          # FilterFunnel / VetoEngine(V2) / DimensionScorer(V2) / MasterScorer
...
系统支持 v1/v2 两套打分体系，通过 `config.py` 中的 `SCORING_VERSION` 全局切换（默认 `"v2"`），或通过 CLI 参数 `--scoring-version v1` 临时切回旧版。
...
# 临时使用 v1
python run_scoring.py --offline-db --scoring-version v1
```

### V2 打分体系（默认，基于巴菲特致股东信+段永平投资问答录）

V2 核心改进：
1. **一致性优先**：ROE/ROIC 连续多年达标比均值更重要（巴菲特"demonstrated consistent earning power"）
2. **FCF 视角**：Owner's Earnings 理念，FCF/营收 > FCF/净利润（段永平"买公司就是买未来现金流折现"）
3. **轻资产偏好**：Capex/营收 作为护城河代理指标（段永平"资本支出大的行业不容易出好企业"）
4. **护城河代理**：毛利率稳定性(CV) + 定价权（利润率趋势）
5. **估值弱化**：FCF收益率优先，避免低PE价值陷阱；估值权重降至10%
6. **成长质量**：营收正增长年数 + 利润率趋势 > 纯CAGR数字

#### V2 五大评分维度

| 维度 | V2权重 | V1权重 | 核心指标（V2） |
|------|--------|--------|--------------|
| 生意质量 | **35%** | 30% | ROE一致性(5年>15%年数)、毛利率均值+CV、FCF/营收、Capex/营收 |
| 财务健康 | 20% | 20% | 净负债/FCF、流动比率、利息覆盖、OCF为正年数 |
| 成长质量 | **15%** | 20% | 营收/EPS CAGR、营收正增长年数、净利率趋势 |
| 资本配置 | **20%** | 15% | ROIC一致性(5年>12%年数)、ROIC均值、股权稀释、股东回报、FCF转化 |
| 估值 | **10%** | 15% | FCF收益率、盈利收益率(1/PE)、PEG、EV/EBITDA |
...
| 芒格 | 35% | 15% | 10% | **35%** | 5% |
| 段永平 | **40%** | 10% | 20% | **30%** | 0% |
| 彼得·林奇 | 20% | 15% | **35%** | 10% | **20%** |

#### V2 一票否决项（VetoEngineV2，更严格）

| 否决项 | V2阈值 | V1阈值 | 来源 |
|--------|--------|--------|------|
| FCF连续为负 | **2年** | 3年 | 巴菲特偏好现金流确定性 |
| 净利润率连续为负 | 2年 | 2年 | 同上 |
| 长期负债/FCF | **>8** | >10 | 巴菲特"little or no debt" |
| 5年均ROE<8% | **新增** | — | 低于资本成本，资本配置差 |
| 毛利率3年下滑>15pct | **新增** | — | 失去定价权/护城河崩塌 |
...
- **V3（完成）**: 打分模块 v2 ✓（基于巴菲特致股东信+段永平投资问答录重新设计打分逻辑：一致性优先、FCF视角、轻资产偏好、护城河代理、估值弱化、成长质量；v1保留可切换）
- **V4**: LLM 读 10-K 做定性分析
- **V5**: 权重自动优化（基于回测 IC 反向调参）
